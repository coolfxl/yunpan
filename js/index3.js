(function(){
	...
	$(".move").on("click",function(){
		...
		for(var i=0; i<moveFile.length; i++){
			var currId = tools.$(".item", moveFile[i])[0].dataset.fileId;
			currIdArr.push(currId);
		}
		for(var key in currIdArr){
			console.log(currIdArr[key]);
		}
		return;*/
		...
		/*else if(moveFile.length >= 2){
			...
			var pophtml = treeHtml(datas, -1);//文件备选区域
			var popup = new PopUp();//打开弹窗
			popup.open({content:pophtml});//这是正确的调用方式
			
			var dirFile = tools.$(".dir-file")[0];//移动文件目标
			var fileName = tools.$(".file-name",dirFile)[0];//文件名字
			var total = tools.$(".total", dirFile)[0];//文件数量
			fileName.innerHTML = tools.$(".file-title", moveFile[0])[0].innerHTML;//在当前对象中找到想要的数据
			total.innerHTML = wholeSelect().length+"个文件";
			var fileMovePathTo = tools.$(".fileMovePathTo")[0];
			fileMovePathTo.innerHTML = "hello";
			
			//为当前的树形目录绑定选中事件
			$(".dirTree").off("click").on("click",".tree-title",function(){
				$(".dirTree").find(".tree-title").removeClass("tree-nav");
