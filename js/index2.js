(function(){
	...
	treeMenu.innerHTML = treeHtml(datas, -1);//渲染属性菜单内容，因为datas中的第一级的pid是-1开始加载，所以设置为-1，后面设置为动态的
		...
		if(!deleteFiles.length){
			...
			$.tmDialog({ //删除提示
				...
				success : function(){ //确定删除
					...
					/*var hashChild = dataControl.hasChilds(datas, getPidInput.value);//当前操作的父级id下是否有子元素  第二种做法
					if(hashChild){//如果有子数据，就渲染出子数据的结构
						//找到当前这个ID下所有的子数据，渲染在文件区域中
						empty.style.display = "none";
						//fileList.innerHTML = createFilesHtml(datas, fileId);//文件内容区域根据树形菜单点击的项进行展示
						...
						empty.style.display = "block";
						fileList.innerHTML = "";//清空fileList里面的内容，只要里面没有内容就清空
						var currentObj = $(".tree-title[data-file-id='"+(getPidInput.value)+"']");
						currentObj.className = "tree-title tree-contro-none";
					}*/
					if(wholeSelect().length == 0){
						tools.removeClass(checkedAll, "checked");
					}
